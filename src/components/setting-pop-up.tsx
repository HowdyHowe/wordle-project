    ...
    <div className="absolute inset-0 flex items-center justify-center z-10">
      ...
      <div className={`relative flex flex-col items-center justify-center w-[300px] h-[450px] rounded-md z-20 px-4 ${darkMode ? "bg-[#2c2c2c]" : "bg-white"}`}>
        <div className="flex flex-col items-center justify-start text-center w-full h-full mt-10 mb-3 overflow-y-auto">
            ...
            <div className="flex flex-row items-center justify-evenly w-full pb-3 border-b">
              <div onClick={() => {
                changeBox()
                dispatch(setBoxCount(5))
              }} className={`flex items-center justify-center w-[30px] h-[30px] ${darkMode ? "border border-white" : "border border-black"} rounded-md ${box === 5 ? "bg-[#dfe2ff] text-black" : ""}`}>5</div>
              <div onClick={() => {
                changeBox()
                dispatch(setBoxCount(6))
              }} className={`flex items-center justify-center w-[30px] h-[30px] ${darkMode ? "border border-white" : "border border-black"} rounded-md ${box === 6 ? "bg-[#dfe2ff] text-black" : ""}`}>6</div>
              <div onClick={() => {
                changeBox()
                dispatch(setBoxCount(7))
              }} className={`flex items-center justify-center w-[30px] h-[30px] ${darkMode ? "border border-white" : "border border-black"} rounded-md ${box === 7 ? "bg-[#dfe2ff] text-black" : ""}`}>7</div>
            </div>
